...
| 11 Personnel | Attributed practice/game-designation snapshots, archived source bytes, scheduled acquisition and matchup context with expiry | Individual report dates, confirmed starter changes, snap-weighted player value and validated numerical effects |
...
| 14 Weather | Fourteen verified US venue locations using Census address points or attributed OSM stadium bounds; five currently eligible games with NWS kickoff-hour context; issue/retrieval timestamps, stale/unknown states, compressed source archives | Remaining domestic/international coverage and validated numerical effects |
