...
class NoLongerDue extends Error {
  constructor(readonly decision: Decision) { super("Collection is no longer due"); }
}
 ...
 */
export async function runPlannedOddsCollection({ collector, readHistory, games }: {
  collector: Omit<Collector, "reuse">;
  readHistory: () => Promise<DecisionInput["history"]>;
  games: DecisionInput["games"];
}) {
  const check = async () => {
    const [history, feed] = await Promise.all([readHistory(), collector.read()]);
    const input = { now: collector.now(), history: history ? { ...history, attempts: [...history.attempts] } : null, feed, games };
    return { input, decision: decideCollection(input) };
  };
  const { decision } = await check();
  if (decision.action !== "collect") return { status: "deferred" as const, decision };
  try {
    return await runOddsCollection({
      ...
      reserve: async () => {
        ...
        if (latest.decision.action !== "collect") throw new NoLongerDue(latest.decision);
        const expiry = await collector.reserve(collector.now());
        ...
        const after = decideCollection({ ...latest.input, now: collector.now() });
        if (after.action !== "collect") throw new NoLongerDue(after);
        return expiry;
      },
    });
  } catch (error) {
    if (error instanceof NoLongerDue) return { status: "deferred" as const, decision: error.decision };
    throw error;
  }
}
