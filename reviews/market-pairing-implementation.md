...
Storage timestamps are authenticated provider metadata, not third-party notarization. The private export stays in the ignored recovery directory. A local report runner now retains a content-addressed checkpoint audit with input, code, protocol, export and capture hashes. Private remote report retention is now verified locally and a workflow is staged; hosted scheduling, a closing capture mechanism, settlement convention and CLV calculation remain unfinished. See `market-audit-automation.md` for storage and replay evidence. The existing schedule cannot promise final-15-minute coverage. No real game has been reported as paired under this implementation.
